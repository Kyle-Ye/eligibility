...
NSString * _Nullable eligibility_input_to_str(uint64_t type) {
    switch (type) {
        ...
        default:
            os_log_error(eligibility_log(), "%s: Unsupported input type: %llu", "eligibility_input_to_str", type);
            return nil;
    }
}
...
NSString * _Nullable eligibility_input_status_to_str(uint64_t status) {
    switch (status) {
        ...
        default:
            os_log_error(eligibility_log(), "%s: Unsupported input status: %llu", "eligibility_input_status_to_str", status);
            break;
    }
    return nil;
}
